...
function findAllPSiblings(where) {
    let listP = where.querySelectorAll('p');
    let result = [];
    for(const el of listP){
...
/*
 ...
 Задачу необходимо решить использования рекурсии, то есть можно не уходить вглубь дерева.
 Так же будьте внимательны при удалении узлов, т.к. можно получить неожиданное поведение при переборе узлов
...
function collectDOMStat(root) {
    ...
    const infoDOM = elms => {
        for (const el of elms.childNodes) {
            if (el.tagName) {
                if (el.tagName in statisticObj.tags) {
                    statisticObj.tags[el.tagName]+=1;
                } else {
                    statisticObj.tags[el.tagName] = 1;
                }
                if (el.classList.length > 0) {
                    el.classList.forEach(val => {
                        if (val in statisticObj.classes) {
                            statisticObj.classes[val]+=1;
                        } else {
                            statisticObj.classes[val] = 1;
                        }
                    });
...
function observeChildNodes(where, fn) {
    let myObs = new MutationObserver(mutations => {
        mutations.forEach(mutations=>{
            if(mutations.addedNodes.length > 0){
                    fn({type:'insert',nodes: [...mutations.addedNodes]});
            }
            if(mutations.removedNodes.length > 0){
                    fn({type:'remove',nodes: [...mutations.removedNodes]});
            }
        });
    });
    myObs.observe(where,{childList:true});
}
